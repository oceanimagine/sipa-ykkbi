...
class get_mata_anggaran_individual_benchmarks extends CI_Model {
    ...
    function get_data(){
        ...
        $sql = "select b.rekmakode as id, b.kode, CONCAT(substring(b.rekmakode from 1 for 3), '.', substring(b.rekmakode from 4 for 3), '.', substring(b.rekmakode from 7 for 2)) rekmakode, b.rekmanama, CONCAT(substring(b.rekmainduk from 1 for 3), '.', substring(b.rekmainduk from 4 for 3), '.', substring(b.rekmainduk from 7 for 2)) rekmainduk, (select a.rekmainduknama from tblmastermainduk a where a.rekmainduk = b.rekmainduk) rekmainduknama, b.benchmarkang, b.benchmarkprog from tblmastermaindividual b ".$clouse.$this->where_project($clouse)." order by id asc, kode asc, rekmainduk asc, rekmakode asc offset $iDisplayStart limit $iDisplayLength";
